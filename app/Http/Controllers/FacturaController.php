...
class FacturaController extends Controller
{
    public function store(Request $request)
   {
      // Validaciones
      $messages = [
           'factura_fecha.required'=>'La fecha de emisión es obligatoria'
           ];
      ...
      $legajo->dominio = $request->input('factura_dominio');
      $legajo->codigo = $request->input('factura_interno');
      ...
      $fecha = str_replace('/', '-', $request->input('factura_fecha'));
      $legajo->fecha = Carbon::createFromFormat("d-m-Y", $fecha);
      $legajo->detalle = $request->input('fact_detalle');
