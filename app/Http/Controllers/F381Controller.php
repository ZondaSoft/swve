...
class F381Controller extends Controller
{
    public function store(Request $request)
    {
        // Validaciones
        $messages = [
            'f381_fecha.required'=>'La fecha de emisión es obligatoria'
            ];
        ...
        $legajo->dominio = $request->input('f381_dominio');
        $legajo->codigo = $request->input('f381_interno');
        $legajo->tramite = 10;   // 1-Libre deuda de multas  2-Libre deuda patentes   3-Informe de dominio  4-Denuncia de venta   5-Verificación polcial  6-Formulario CETA

        $fecha = str_replace('/', '-', $request->input('f381_fecha'));
        $legajo->fecha = Carbon::createFromFormat("d-m-Y", $fecha);
        $legajo->detalle = $request->input('f381_detalle');
    ...
    {
        // Validaciones
        if ($request->input('btngrabarF381') == 'grabar') {
            $messages = [
                'f381_ed_fecha.required'=>'La fecha es obligatoria'
                ];

            $rules = [
                'f381_ed_fecha'=>'required'
                ];
        ...
        $fecha = str_replace('/', '-', $request->input('f381_ed_fecha'));
        $legajo->fecha = Carbon::createFromFormat("d-m-Y", $fecha);
        $legajo->detalle = $request->input('f381_ed_detalle');
